...
Layer::Layer(int input, int output, float (*activation)(float), float(*activation_derivative)(float)) : input_size(input), output_size(output), weights(output, input), bias(output, 1), activation(activation), activation_derivative(activation_derivative)
{
...
Matrix Layer::forward(const Matrix &input)
{
  ...
  Matrix output = z.apply(this->activation);
...
{
  Matrix activation_mask = last_z.apply(this->activation_derivative);
  Matrix dL_dz = gradient.elementwise_multiply(activation_mask);
